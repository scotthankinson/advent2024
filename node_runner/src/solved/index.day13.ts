...
const findMinimumTokensWithBigInt = (machine: ClawMachine): number | null => {
    ...
    console.log(`\nAnalyzing machine movements:`);
    console.log(`Button A: (${buttonAX}, ${buttonAY})`);
    console.log(`Button B: (${buttonBX}, ${buttonBY})`);
    console.log(`Target: (${prizeX}, ${prizeY})`);

    // Check X coordinate divisibility
    const gcdX = gcd(buttonAX, buttonBX);
    if (prizeX % gcdX !== 0n) {
        console.log(`X coordinate ${prizeX} not achievable - not divisible by GCD(${buttonAX}, ${buttonBX}) = ${gcdX}`);
        return null;
    }

    // Check Y coordinate divisibility
    const gcdY = gcd(buttonAY, buttonBY);
    if (prizeY % gcdY !== 0n) {
        console.log(`Y coordinate ${prizeY} not achievable - not divisible by GCD(${buttonAY}, ${buttonBY}) = ${gcdY}`);
        return null;
    }
    ...
    if (det === 0n) {
        console.log('Buttons move in parallel - no unique solution');
        return null;
    }
    ...
    // Calculate solution
    const aNumerator = (prizeX * buttonBY) - (prizeY * buttonBX);
    const bNumerator = (buttonAX * prizeY) - (buttonAY * prizeX);

    console.log(`a numerator: ${aNumerator}`);
    console.log(`b numerator: ${bNumerator}`);

    // Check if these divide evenly by determinant
    if (aNumerator % det !== 0n || bNumerator % det !== 0n) {
        console.log('No integer solution exists');
        return null;
    }

    const a = aNumerator / det;
    const b = bNumerator / det;

    console.log(`Calculated a: ${a}`);
    console.log(`Calculated b: ${b}`);

    // Verify solution
    const checkX = (buttonAX * a) + (buttonBX * b);
    const checkY = (buttonAY * a) + (buttonBY * b);

    console.log(`Verification - Target: (${prizeX}, ${prizeY})`);
    console.log(`Verification - Reached: (${checkX}, ${checkY})`);

    // For Part 2, we only need positive solutions, no press limit
    if (a >= 0n && b >= 0n) {
        const tokens = Number((3n * a) + b);
        console.log(`Valid solution found: Button A: ${a} presses, Button B: ${b} presses`);
